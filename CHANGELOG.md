...
### Fixed
- Localhost navigation now bypasses Kuri's public-URL SSRF guard through the active browser tab.
- Kuri startup is serialized, retries transient failures, recovers after child exit, and uses isolated Chrome profiles.
- Device emulation uses Kuri's correct `ua` parameter, and screenshots no longer advertise unsupported element-reference cropping.
- Gemini resolves the MCP server relative to the installed extension and correctly exposes settings.
- Screenshot writes and browser inputs are bounded and workspace-constrained.

### Changed
- Setup now requires Node.js 22.2.0+ and Kuri 0.4.1+, uses locked dependencies, and detects stale builds.
- Browser-agent guidance now requires explicit tab targeting for parallel tasks.
